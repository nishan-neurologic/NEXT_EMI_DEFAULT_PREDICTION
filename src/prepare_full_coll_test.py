...
#     actual_default = account_data.iloc[-1]["Default"]

#     # prediction = model.predict_proba(input_data_flat)
...
#     # return np.argmax(prediction), result, input_data_flat, actual_default
#     return result["PREDICTION"], result, input_data_flat, actual_default

def predict_default(account_id, data, cat_features, COLUMNS_TO_SCALE, encoder, scaler, model, input_timesteps=5, output_timestep=1):
    result = dict()
    account_data = data[data['ACCOUNT_ID'] == account_id]
    ...
    account_sequence = preprocessed_data.values
    if len(account_sequence) < input_timesteps:
        account_sequence = pad_sequences([account_sequence], maxlen=input_timesteps, dtype='float32', padding='post')[0]

    train_x = []
    train_y = []

    for i in range(len(account_sequence) - input_timesteps - output_timestep + 1):
        train_x.append(account_sequence[i:i + input_timesteps, :-1])
        train_y.append(account_sequence[i + input_timesteps, -1])
    ...
    input_data_list = []
    predictions = []
    for input_data in train_x:
        input_data = np.expand_dims(input_data, axis=0).astype('float32')
        input_data_flat = input_data.reshape(input_data.shape[0], -1)
        prediction = model.predict_proba(input_data_flat)
        predictions.append(bool(np.argmax(prediction)))
        input_data_list.append(input_data_flat[0])

    return predictions, train_y, input_data_list
...
# def predict_default_wrapper(account_ids, data, CAT_FEATURES, COLUMNS_TO_SCALE, encoder, scaler, model, output_csv_file):
#     result = dict()
#     flattened_data_list = []
#     prediction_list = []
#     actual_default_list = []
#     for account_id in account_ids:
#         pred, result[account_id], flattened_data, actual_default = predict_default(account_id, data, CAT_FEATURES, COLUMNS_TO_SCALE, encoder, scaler, model)
#         flattened_data_list.append(flattened_data[0])
#         prediction_list.append(pred)
#         actual_default_list.append(actual_default)

#     flattened_data_df = pd.DataFrame(flattened_data_list, columns=data_columns * 5)
#     flattened_data_df['PREDICTION'] = prediction_list
#     flattened_data_df['Default'] = actual_default_list

#     flattened_data_df.to_csv(output_csv_file, index=False)
...
def predict_default_wrapper(account_ids, data, CAT_FEATURES, COLUMNS_TO_SCALE, encoder, scaler, model, output_csv_file):
    ...
    for account_id in account_ids:
        predictions, actual_defaults, input_data_list = predict_default(account_id, data, CAT_FEATURES, COLUMNS_TO_SCALE, encoder, scaler, model)
        all_predictions.extend(predictions)
        all_actual_defaults.extend(actual_defaults)
        all_input_data.extend(input_data_list)

    data_columns = data.columns[:-1].tolist() * 5
    data_columns.append('PREDICTION')
    data_columns.append('Default')

    result_df = pd.DataFrame(all_input_data, columns=data_columns[:-2])
    result_df['PREDICTION'] = all_predictions
    result_df['Default'] = all_actual_defaults
...
if __name__=="__main__":
    ...
    loaded_model = xgb.XGBClassifier()
    loaded_model.load_model(MODEL_PATH)
    input_csv_file = "/Users/nishanali/WorkSpace/arthan_emi_default_prediction/data/processed/merged.csv"
    output_csv_file = "/Users/nishanali/WorkSpace/arthan_emi_default_prediction/data/processed/output.csv"
    # data = pd.read_csv(PROCESSED_DATA_PATH)
    ...
    prediction = predict_default_wrapper(account_ids, data, CAT_FEATURES, COLUMNS_TO_SCALE, encoder, scaler, loaded_model, output_csv_file)
    print(f"The predicted default status for account ID {account_ids} is: {prediction}")
    print(f"The predicted default status for account ID {account_ids} is: {prediction}")
